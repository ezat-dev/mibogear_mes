...
.detail {
	...
	width: 650px; /* 가로 길이 고정 */
	height: 330px; /* 세로 길이 고정 */
...
<form method="post" class="corrForm" id="sparePartForm" name="sparePartForm">	    
<div class="sparePartModal">    
 <div class="detail">
 <div class="header">
 	스페어등록
    ...
    <table class="product-table">
        ...
        <tbody>
            <tr>
                ...
                <th>품명</th>
                <td><input type="text" id="spp_name" name="spp_name"></td>
            </tr>
            <tr>
                <th>품번</th>
                <td><input type="text" id="spp_no" name="spp_no"></td>
                <th>규격</th>
                <td><input type="text" id="spp_gyu" name="spp_gyu"></td>
            </tr>
            <tr>
                <th>교체주기</th>
                <td><input type="text" id="spp_yong" name="spp_yong"></td>
                <th>적정재고</th>
                <td><input type="text" id="spp_proper" name="spp_proper"></td>
            </tr>
        </tbody>
    </table>
...
<script>
	...
	function getSparePartList(){
		userTable = new Tabulator("#tab1", {
		    ...
		    columns:[
		        {title:"코드", field:"spp_code", sorter:"string", width:120,
			        hozAlign:"center", headerFilter:"input", visible:false},
				{title:"거래처명", field:"corp_name", sorter:"string", width:200,
				    hozAlign:"center", headerFilter:"input"}, 
				{title:"품명", field:"spp_name", sorter:"string", width:200,
				    hozAlign:"center", headerFilter:"input"}, 
		        {title:"품번", field:"spp_no", sorter:"string", width:200,
		        	hozAlign:"center", headerFilter:"input"},		        
		        {title:"규격", field:"spp_gyu", sorter:"string", width:200,
		        	hozAlign:"center", headerFilter:"input"},
		        {title:"교체주기", field:"spp_yong", sorter:"string", width:200,
		        	hozAlign:"center", headerFilter:"input"},
		        	{title:"적정재고", field:"spp_proper", sorter:"string", width:200,
			        	hozAlign:"center", headerFilter:"input"},
...
/* 				sparePartDetail(data.prod_code);	
				 $('.delete').show();  */
    ...
    $.ajax({
        ...
        success: function (result) {
           ...
           for (let key in d) {
            ...
            // 모달 열기
            $('.sparePartModal').show().addClass('show');
        },
 ...
 // 드래그 기능 추가
	const modal = document.querySelector('.sparePartModal');
	...
	const sparePartModal = document.querySelector('.sparePartModal');
	const closeButton = document.querySelector('.close');
	const headerCloseButton = document.querySelector('.header-close');

	insertButton.addEventListener('click', function() {
		isEditMode = false;  // 추가 모드
	    $('#sparePartForm')[0].reset(); // 폼 초기화
		
	    sparePartModal.style.display = 'block'; // 모달 표시

		$('.delete').hide();
	});

	closeButton.addEventListener('click', function() {
		sparePartModal.style.display = 'none'; // 모달 숨김
	});
...
//				$("#tab1 .tabulator-col.tabulator-sortable").css("height","55px");
  ...
  //제품등록 저장
   function save() {
    ...
    let confirmMsg = "";
    if (isEditMode && selectedRowData && selectedRowData.spp_code) {
        formData.append("mode", "update");
        formData.append("spp_code", selectedRowData.spp_code);
        confirmMsg = "수정하시겠습니까?";
    } else {
   ...
   /* function saveAsNew() {
    ...
    function deleteSparePart() {
	    if (!selectedRowData || !selectedRowData.spp_code) {
	        alert("삭제할 대상을 선택하세요.");
	        return;
	    }
